...
static int create_and_bind_tcp(const char *host, const char *port)
{
    ...
    if (rp == NULL)
    {
        perror("Could not bind") return -1;
    }
    freeaddrinfo(result);
    return sfd;
}
...
ssize_t send_bytes(int fd, const unsigned char *buf, size_t len)
{
    ...
    return total;
err:
    fprintf(stderr, "send - error sending data: %s", strerror(errno));
    return -1;
}
...
ssize_t recv_bytes(int fd, unsigned char *buf, size_t bufsize)
{
    ...
    return total;
err:
    fprintf(stderr, "recv - error receiving data: %s", strerror(errno));
    return -1;
}
...
void evloop_add_periodic_task(struct evloop *loop,
                              int seconds, unsigned long long ns, struct closure *cb)
{
    ...
    loop->periodics_tasks[loop->periodic_nr] = malloc(sizeof(*loop->periodic_tasks[loop->periodic_nr]));
